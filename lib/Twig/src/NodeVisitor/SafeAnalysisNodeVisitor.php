 ...
 * @final
 */
class SafeAnalysisNodeVisitor extends \ToolsetAdvancedExport\Twig\NodeVisitor\AbstractNodeVisitor
{
    protected $data = [];
    protected $safeVars = [];
    ...
    public function getSafe(\ToolsetAdvancedExport\Twig_NodeInterface $node)
    {
    ...
    protected function setSafe(\ToolsetAdvancedExport\Twig_NodeInterface $node, array $safe)
    {
        $hash = \spl_object_hash($node);
        if (isset($this->data[$hash])) {
            foreach ($this->data[$hash] as &$bucket) {
    ...
    protected function doLeaveNode(\ToolsetAdvancedExport\Twig\Node\Node $node, \ToolsetAdvancedExport\Twig\Environment $env)
    {
        if ($node instanceof \ToolsetAdvancedExport\Twig\Node\Expression\ConstantExpression) {
            ...
            $this->setSafe($node, ['all']);
        } elseif ($node instanceof \ToolsetAdvancedExport\Twig\Node\Expression\ParentExpression) {
            // parent block is safe by definition
            $this->setSafe($node, ['all']);
        } elseif ($node instanceof \ToolsetAdvancedExport\Twig\Node\Expression\ConditionalExpression) {
        ...
        } elseif ($node instanceof \ToolsetAdvancedExport\Twig\Node\Expression\FunctionExpression) {
        ...
        } elseif ($node instanceof \ToolsetAdvancedExport\Twig\Node\Expression\MethodCallExpression) {
            if ($node->getAttribute('safe')) {
                $this->setSafe($node, ['all']);
            } else {
                $this->setSafe($node, []);
            }
        } elseif ($node instanceof \ToolsetAdvancedExport\Twig\Node\Expression\GetAttrExpression && $node->getNode('node') instanceof \ToolsetAdvancedExport\Twig\Node\Expression\NameExpression) {
...
\class_alias('ToolsetAdvancedExport\\Twig\\NodeVisitor\\SafeAnalysisNodeVisitor', 'ToolsetAdvancedExport\\Twig_NodeVisitor_SafeAnalysis');
