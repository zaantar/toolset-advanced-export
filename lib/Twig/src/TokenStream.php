...
class TokenStream
{
    ...
    public function __construct(array $tokens, $name = null, $source = null)
    {
        if (!$name instanceof \ToolsetAdvancedExport\Twig\Source) {
            ...
            $this->source = new \ToolsetAdvancedExport\Twig\Source($source, $name);
        } else {
            $this->source = $name;
        }
        $this->tokens = $tokens;
    ...
    public function next()
    {
        if (!isset($this->tokens[++$this->current])) {
            throw new \ToolsetAdvancedExport\Twig\Error\SyntaxError('Unexpected end of template.', $this->tokens[$this->current - 1]->getLine(), $this->source);
        }
    ...
    public function expect($type, $value = null, $message = null)
    {
        ...
        if (!$token->test($type, $value)) {
            $line = $token->getLine();
            throw new \ToolsetAdvancedExport\Twig\Error\SyntaxError(\sprintf('%sUnexpected token "%s"%s ("%s" expected%s).', $message ? $message . '. ' : '', \ToolsetAdvancedExport\Twig\Token::typeToEnglish($token->getType()), $token->getValue() ? \sprintf(' of value "%s"', $token->getValue()) : '', \ToolsetAdvancedExport\Twig\Token::typeToEnglish($type), $value ? \sprintf(' with value "%s"', $value) : ''), $line, $this->source);
        }
        $this->next();
        return $token;
    }
    ...
    public function look($number = 1)
    {
        if (!isset($this->tokens[$this->current + $number])) {
            throw new \ToolsetAdvancedExport\Twig\Error\SyntaxError('Unexpected end of template.', $this->tokens[$this->current + $number - 1]->getLine(), $this->source);
        }
    ...
    {
        return \ToolsetAdvancedExport\Twig\Token::EOF_TYPE === $this->tokens[$this->current]->getType();
    }
    /**
     * @return Token
     */
...
\class_alias('ToolsetAdvancedExport\\Twig\\TokenStream', 'ToolsetAdvancedExport\\Twig_TokenStream');
